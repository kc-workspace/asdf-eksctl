...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    ...
    url="https://github.com/eksctl-io/eksctl/releases/download/v{version}/eksctl_{os}_{arch}.{ext}"
  ...
  if [[ "$mode" == "custom" ]]; then
    ...
    if kc_asdf_enabled_feature checksum; then
      local checksum_url
      checksum_url="https://github.com/eksctl-io/eksctl/releases/download/v{version}/eksctl_checksums.txt"
